...
- [x] Vault persistence (#15): last vault saved to app_data_dir, auto-reopens on launch
- [x] Incremental backlinks index (#6): re-scan only changed file instead of full tree
- [x] Pathspec-limited `get_note_history` diff (#7): O(commits × files) → O(commits × 1)
- [x] Loading indicators (#12): spinners for openFile, refreshFileTree, loadGraph, create/delete

## Phase 3 — Power Features

- [ ] Note templates — create notes from user-defined templates with `{{date}}` placeholders
- [ ] Quick preview — hover over a file/folder to see a preview (or h一个新的aside panel)
